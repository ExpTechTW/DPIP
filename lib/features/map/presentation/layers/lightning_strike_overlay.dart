/// The lightning-strike marks themselves — the MapLibre source, symbol layer,
/// baked icons and snapshot cache that draw one 閃電 frame.
///
/// Split out of [LightningMapLayer] because the strikes have two homes now: the
/// standalone 閃電 timeline layer, and the radar echo's own lightning overlay,
/// which draws the snapshot matching whichever radar frame is on screen. Both
/// need the identical marks — the same age ramp, the same baked PNGs, the same
/// legend — so the drawing lives here once and each host supplies only *which*
/// frame to show.
///
/// Colour is age vs the frame clock (5 / 10 / 30 / 60 min); shape is type
/// (circle = cloud-to-cloud, cross = cloud-to-ground) — legacy look without map
/// sprites.
library;
...
/// Draws lightning strikes on one map surface.
///
/// [namespace] scopes the MapLibre source/layer ids, so the 閃電 layer and the
/// radar overlay can each own a mount without colliding over one id. The baked
/// icons deliberately stay on shared ids: they are style-global, identical, and
/// registering one set twice would only cost memory.
class LightningStrikeOverlay {
  LightningStrikeOverlay(this._repository, {String namespace = 'lightning'})
  ...
  /// The strike key, in the order the legend reads it.
  static List<SymbolLegendItem> legendItems(BuildContext context) {
  ...
  Future<void> show(
    ...
    try {
      ...
      Log.handle(error, stackTrace, 'lightning show $frameId');
    }
  ...
  /// Mounts the layer with no strikes on it — for a host whose current frame
  /// has no lightning snapshot near enough to be honest about. Clearing the
  /// features rather than removing the layer keeps "the overlay is on, there
  /// is nothing to draw" distinct from "the overlay is off".
  Future<void> showEmpty(MapLibreMapController controller) async {
    ...
    try {
      ...
      Log.handle(error, stackTrace, 'lightning clear features');
    }
  }
