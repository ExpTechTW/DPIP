              ...
              children: [
                for (var i = 0; i < sections.length; i++) ...[
                  ...
                  if (sections[i].title == 'Network usage' &&
                      _usageHistory != null)
                    _NetworkUsageChart(history: _usageHistory!),
                ],
...
/// Hourly downloaded-vs-saved trend for the Network usage section.
///
/// One stacked bar per hour over the trailing 24 h (Taipei wall-clock on the
/// x-axis): the blue segment is what the app actually downloaded, the green
/// segment what the cache saved on top — so the shape of the day and the cache
/// payoff read together. Deliberately matches the section's English-only
/// convention.
class _NetworkUsageChart extends StatelessWidget {
  const _NetworkUsageChart({required this.history});
  ...
  Widget build(BuildContext context) {
    ...
    final maxBytes = history.fold<int>(0, (m, h) => math.max(m, h.down + h.saved));
    if (maxBytes == 0) {
      return Padding(
